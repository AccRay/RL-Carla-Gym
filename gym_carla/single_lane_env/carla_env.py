...
from gym_carla.single_lane_env.tools.misc import draw_waypoints, get_speed, get_acceleration, test_waypoint, \
    get_actor_polygons, get_lane_center, remove_unnecessary_objects, get_yaw_diff, create_vehicle_blueprint
from gym_carla.single_lane_env.tools.sensor import CollisionSensor, LaneInvasionSensor, SemanticTags
from gym_carla.single_lane_env.gym_carla_agent.local_rl_planner import LocalRlPlanner
from gym_carla.single_lane_env.gym_carla_agent.custom_route_planner import CustomRoutePlanner
from gym_carla.single_lane_env.carla_agent.behavior_agent import BasicAgent

import pygame
from gym_carla.single_lane_env.tools.render import PygameWorld, HUD
...
class CarlaEnv:
    def __init__(self, args) -> None:
        ...
        self.global_planner = CustomRoutePlanner(self.map, self.sampling_resolution)
    ...
    def reset(self):
        ...
        self.local_planner = LocalRlPlanner(self.ego_vehicle,
    ...
    def step(self, action):
        ...
        if self.is_effective_action():
            ...
            return state, reward, self._truncated(), self._done(), self._get_info(
                {'Steer': self.control.steer, 'Throttle': self.control.throttle,
    ...
    def _get_state(self, dict):
        ...
        if dict['vehicle_front']:
            ...
            vehicle_len = max(abs(self.ego_vehicle.bounding_box.extent.x),
                          ...
                          max(abs(self.vehicle_front.bounding_box.extent.x),
                              abs(self.vehicle_front.bounding_box.extent.y))
    ...
    def _get_reward(self):
        ...
        self.step_info = {'velocity': v_s, 'offlane': Lcen, 'yaw_diff': yaw_diff, 'TTC': TTC, 'fTTC': fTTC,
                          'Comfort': fCom, 'Efficiency': fEff, 'Lane_center': fLcen, 'Yaw': fYaw, 'Abandon': False}
    ...
    def _init_renderer(self):
        ...
        self.world = PygameWorld(self.sim_world, self.hud)
