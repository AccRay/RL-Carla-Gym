...
from rl_algs.tools.replay_buffer import ReplayBuffer
...
class PolicyNet(torch.nn.Module):
    ...
    def forward(self, state):
        ...
        state_ev = state[:, -self.state_dim['companion_vehicle'] - self.state_dim['ego_vehicle']:-self.state_dim[
            'companion_vehicle']]
...
class DDPG:
    ...
    def take_action(self, state):
        ...
        if self.a_dim == 3:
            if (action[0, 0].is_cuda):
                action = np.array([action[:, 0].detach().cpu().numpy(), action[:, 1].detach().cpu().numpy(),
                                   action[:, 2].detach().cpu().numpy()]).reshape((-1, 3))
            else:
                action = np.array([action[:, 0].detach().numpy(), action[:, 1].detach().numpy(),
                                   action[:, 2].detach().numpy()]).reshape((-1, 3))
            print(f"Network Output - Steer: {action[0][0]}, Throttle_brake: {action[0][1]}, Exec_steps:{action[0][2]}")
        else:
            if (action[0, 0].is_cuda):
                action = np.array([action[:, 0].detach().cpu().numpy(), action[:, 1].detach().cpu().numpy()]).reshape(
                    (-1, 2))
            else:
    ...
    def store_transition(self, state, action, reward, next_state, truncated, done,
                         info):  # how to store the episodic data to buffer
        def _compress(state):
